    ...
    if params.get("sql_create_table"):
        ...
        for column in rows[0]:
            ...
            if item.get("primary_key"):
                ...
                definition += " UNIQUE"
            if item.get("nullable", True) is False or float(item.get("nullable_rate", 0) or 0) == 0 and item.get("required"):
                definition += " NOT NULL"
...
class Plugin:
    ...
    def _rules(self, params: dict) -> list[dict]:
        ...
        modes = [key for key in ("fields", "rules", "rule_set", "source_file", "template") if key in params and params.get(key) not in (None, "", [])]
        if len(modes) > 1:
