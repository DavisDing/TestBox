...
class ReleaseVersionTests(unittest.TestCase):
    ...
    def test_repository_release_version_declarations_are_consistent(self):
        ...
        version_declarations = {
            ...
            "installer": re.findall(
                r'^#define AppVersion "([^"]+)"$',
                (root / "installer" / "TestBox.iss").read_text(encoding="utf-8"),
                re.MULTILINE,
            ),
            "updater": re.findall(
                r'^#define AppVersion "([^"]+)"$',
                (root / "installer" / "TestBoxUpdate.iss").read_text(encoding="utf-8"),
                re.MULTILINE,
            ),
            "update_package": re.findall(
                r'^#define UpdatePackage "TestBox-update-v([^"]+)\.zip"$',
                (root / "installer" / "TestBoxUpdate.iss").read_text(encoding="utf-8"),
                re.MULTILINE,
            ),
        }
    ...
    def test_sync_updates_all_release_version_declarations(self):
        with tempfile.TemporaryDirectory() as temp_name:
            ...
            self.assertEqual(len(changed), 4, "five declarations span four version files")
